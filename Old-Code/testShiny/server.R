...
output$overview <- renderTable(
{
  ...
  {
    with(unique(df[,c("line","individualPlant","phenotype","experiment","treatment","facility","value")]),
         aggregate(cbind(value=value),by=list(Accession=line,phenotype=phenotype,experiment=experiment,treatment=treatment),mean))
  }
  else
  {
    unique(df[,c("line","individualPlant","phenotype","experiment","treatment","facility","value")])
  }
})
...
output$exptplot <- renderPlot({
  data = values()
  data = data[which(data$line == input$line),1:4]
  mdata = melt(data, id=c('phenotype','treatment','experiment'))
  ggplot(mdata, aes(value)) + geom_histogram() + facet_wrap(experiment+phenotype+treatment~variable, scales = 'free', ncol=3)
})
